...
& ChatGPT~4.0 \\
\hline
Linguistic proofreading of the full manuscript
& Grammarly \\
\hline
\end{tabular}
\end{center}
...
\]
After grouping neighbours by species, each species
...
\texttt{sel} = [N_c^{(0)}, N_c^{(1)}, \dots],
\]
yielding for every atom a geometric matrix of identical shape
\[
R_i \in \mathbb R^{N_c^{\mathrm{(total)}} \times 4}.
\]

\subsubsection{Feature normalization (this work)}
...
\begin{itemize}
    \item the \textit{fitting-network derivative}
          $\partial E_i / \partial D_i$
          (forward-mode AD),
    \item the \textit{descriptor-level derivatives}
          $\partial D_i / \partial \hat{R}_i$ and $\partial D_i / \partial G_i$,
    \item the \textit{embedding-network sensitivity to geometry}
          $\partial G_i / \partial \hat{s}_i$,
    \item the \textit{projection} $\partial \hat{s}_i / \partial \hat{R}_i$,
    \item the \textit{analytic geometric derivatives}
