...
DeepMD requires that all atomic coordinates be expressed inside the
primary simulation cell before any descriptor quantities are computed.
Unlike the minimum-image convention used in classical molecular
dynamics, which only enforces periodicity for relative displacements,
DeepMD operates on \emph{absolute atomic coordinates}.  
Therefore each atomic position must be mapped explicitly into the
reference simulation cell.

For a general (possibly triclinic) simulation cell with lattice matrix
$\mathbf{H} = [\mathbf{a}, \mathbf{b}, \mathbf{c}]$, the wrapping is
performed in fractional coordinates.  
Given a Cartesian position $\mathbf{r}_i$, the corresponding fractional
coordinate is

\begin{equation}
\mathbf{s}_i = \mathbf{r}_i \mathbf{H}^{-1}.
\end{equation}

Periodicity is enforced by applying a modulo operation in fractional
space,

\begin{equation}
\tilde{\mathbf{s}}_i = \mathbf{s}_i - \lfloor \mathbf{s}_i \rfloor,
\end{equation}

which ensures $\tilde{s}_{i\alpha} \in [0,1)$ for each lattice direction
$\alpha \in \{a,b,c\}$.  
The wrapped Cartesian coordinate is then reconstructed as

\begin{equation}
\tilde{\mathbf{r}}_i = \tilde{\mathbf{s}}_i \mathbf{H}.
\end{equation}

This procedure is equivalent to the simple Cartesian formula
$\tilde r_{i\alpha} = r_{i\alpha} - L_\alpha \lfloor r_{i\alpha} / L_\alpha \rfloor$
only in the special case of an orthorhombic cell.  
For general simulation cells the fractional-coordinate formulation is
required and is exactly the procedure implemented in the DeepMD input
pipeline.
...
% % \tikzstyle{every node}=[font=\fontsize{4}{4}\selectfont]
% \tikzstyle{every node}=[font=\tiny]
...
% % --- Real atoms r_i ---
% \draw [dashed] (-0.75,0.75) circle (0.08cm) node[left] {$r_0$};
% \draw [dashed] (0.75,0.75) circle (0.08cm) node[right] {$r_3$};
% \draw [dashed] (-0.75,-0.75)  circle (0.08cm) node[left] {$r_1$};
% \draw [dashed] (1.25,-0.25) circle (0.08cm) node[right] {$r_2$};

% % --- Fractional coordinates s_i ---
% \draw [dashed] (-0.375,0.375) circle (0.08cm) node[left] {$s_0$, $\tilde s_0$};
% \draw [dashed] (0.375,0.375)  circle (0.08cm) node[below] {$s_3$, $\tilde s_3$};
% \draw [dashed] (-0.375,-0.375)  circle (0.08cm) node[left] {$s_1$,$\tilde s_1$};
% \draw [dashed] (0.625,-0.125) circle (0.08cm) node[right] {$s_2$};

% % % % --- Wrapped fractional coordinates \tilde s_i ---
% % \draw (0.625,0.375) circle (0.08cm) node[above] {$\tilde s_0$};
% % \draw (0.375,0.375) circle (0.08cm) node[right] {$\tilde s_3$};
% % \draw (0.625,0.625) circle (0.08cm) node[right] {$\tilde s_1$};
% \draw (0.125,-0.125)   circle (0.08cm) node[above] {$\tilde s_2$};


% \draw (0.25,-0.25)   circle (0.08cm) node[above] {$\tilde r_2$};


% % --- Mapping arrows ---
% \draw[->, thin, gray] (-0.75,0.75) -- (-0.375,0.375);
% \draw[->, thin, gray] (0.75,0.75) -- (0.375,0.375);
% \draw[->, thin, gray] (-0.75,-0.75) -- (-0.375,-0.375);
% \draw[->, thin, gray] (1.25,-0.25) -- (0.625,-0.125);

% % \draw[->, thick, blue] (-0.375,0.375) -- (0.625,0.375);
% % \draw[->, thick, blue] (-0.375,-0.375) -- (0.625,0.625);
% \draw[->, thin, blue] (0.625,-0.125) -- (0.125,-0.125);

% \draw[->, thin, black] (0.125,-0.125) -- (0.25,-0.25);
% \draw[->, thin, black] (-0.375,0.375) -- (-0.75,0.75);
% \draw[->, thin, black] (0.375,0.375) -- (0.75,0.75);
% \draw[->, thin, black] (-0.375,-0.375) -- (-0.75,-0.75);
...
% % --- Dimensions ---
% \draw [<->, >=Stealth] (-1,1) -- (-1,-1) node[pos=0.5, fill=white] {$L_y=2$};
% \draw [<->, >=Stealth] (-1,1) -- (1,1) node[pos=0.5, fill=white] {$L_x=2$};

% \draw [<->, >=Stealth] (-0.5,0.5) -- (-0.5,-0.5) node[pos=0.5, fill=white] {1};
% \draw [<->, >=Stealth] (-0.5,0.5) -- (0.5,0.5) node[pos=0.5, fill=white] {1};

% % % --- Column headers ---
% % \node at (-0.8,1.2) {\textbf{Real}};
% % \node at (-0.05,1.2) {\textbf{Fractional}};
% % \node at (0.8,1.2) {\textbf{Wrapped}};
